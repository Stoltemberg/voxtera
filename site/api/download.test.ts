...
describe("download handler", () => {
  ...
  it("redirects a valid HTTPS launcher asset without exposing a GitHub token", async () => {
    fetchMock.mockResolvedValue(new Response(JSON.stringify({
      assets: [{
        name: "VoxteraLauncher.exe",
        browser_download_url: "https://github.com/Stoltemberg/voxtera/releases/download/v1/VoxteraLauncher.exe",
      }],
    }), { status: 200 }));
    ...
    expect(redirects).toEqual([{
      statusCode: 307,
      url: "https://github.com/Stoltemberg/voxtera/releases/download/v1/VoxteraLauncher.exe",
    }]);
    expect(fetchMock).toHaveBeenCalledWith(LATEST_RELEASE_URL, expect.objectContaining({
  ...
  ])("redirects to release history when the launcher asset is unavailable or unsafe", async (release) => {
  ...
  it("redirects to release history when the GitHub request times out", async () => {
    ...
    const { redirects, response } = createResponse();
    const request = (await loadHandler())({}, response);
  ...
  it("uses a cached resolved redirect for subsequent downloads", async () => {
    ...
    await handler({}, first.response);
    await handler({}, second.response);

    expect(first.redirects).toEqual([{
      statusCode: 307,
      url: "https://github.com/Stoltemberg/voxtera/releases/download/v1/VoxteraLauncher.exe",
    }]);
    expect(second.redirects).toEqual(first.redirects);
