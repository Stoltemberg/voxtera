hud-map-map_title = Mapa
hud-map-qlog_title = Missões
hud-map-topo_map = Topografia
hud-map-difficulty = Dificuldade
hud-map-towns = Cidades
hud-map-castles = Castelos
hud-map-dungeons = Masmorras
hud-map-caves = Cavernas
hud-map-cave = Caverna
hud-map-peaks = Montanhas
hud-map-biomes = Biomas
hud-map-voxel_map = Mapa Voxel
hud-map-trees = Árvores Gigantes
hud-map-tree = Árvore Gigante
hud-map-town = Cidade
hud-map-castle = Castelo
hud-map-bridge = Ponte
hud-map-bridges = Pontes
hud-map-glider_course = Pista de Planador
hud-map-glider_courses = Pistas de Planador
hud-map-dungeon = Masmorra
hud-map-quests = Missões
hud-map-df_mine = Mina
hud-map-plaza = Praça
hud-map-workshop = Oficina
hud-map-airship_dock = Doca de Dirigíveis
hud-map-character = Personagem
hud-map-character-label = Última localização conhecida de { $name }
hud-map-creature-label = Última localização conhecida de { $body }
hud-map-escort-label = Escoltar { $name } até { $place }.
hud-map-difficulty_dungeon =
    Dificuldade da

    Masmorra: { $difficulty }
hud-map-drag = Arrastar
hud-map-zoom = Zoom
hud-map-mid_click = Adicionar marcador
hud-map-recenter = Recentralizar
hud-map-marked_location = Localização Marcada
hud-map-marked_location_remove = Clique para remover
hud-map-change_map_mode = Alterar modo do mapa
hud-map-toggle_minimap_voxel = Alternar Modo Voxel no Minimapa
hud-map-zoom_minimap_explanation =
    Dê zoom no minimapa para ver
    sua área em maior detalhes
hud-map-gnarling = Fortificação Retorcida
hud-map-chapel_site = Capela Marítima
hud-map-placed_by = Marcado por { $name }
hud-map-adlet = Fortaleza Adlet
hud-map-terracotta = Ruínas de Terracota
hud-map-df_mine = Mina
hud-map-haniwa = Catacumba Haniwa
hud-map-unknown = Desconhecido
hud-map-cultist = Masmorra Cultista
hud-map-sahagin = Ilha Sahagin
hud-map-myrmidon = Masmorna Mirmidão
hud-map-vampire_castle = Castelo de Vampiros
